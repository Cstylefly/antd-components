...
import BraftEditor, {EditorState, ControlType, ExtendControlType, ImageControlType, HooksType} from 'braft-editor'
...
const RichTextEditor:React.FC<RichTextEditorProps> = (props) => {
  ...
  //扩展功能
  const [finallyExtendControls,setFinallyExtendControls] = useState<ExtendControlType[]>([])
  //----------预览功能----------
  const preview = () => {
    console.log(preview)
  ...
  useEffect(() => {
    if(props?.showPreview){
      props?.extendControls?.length ? setFinallyExtendControls([...preViewExtendControls,...props?.extendControls]) : setFinallyExtendControls(preViewExtendControls)
    }else {
